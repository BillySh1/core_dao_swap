...
const VaultStakeModal: React.FC<VaultStakeModalProps> = ({
  ...
  const handleWithdrawal = async () => {
    ...
    const receipt = await fetchWithCatchTxError(() => {
      ...
      return isWithdrawingAll
        ? callWithGasPrice(vaultPoolContract as any, 'withdrawAll', undefined, callOptions)
        : callWithGasPrice(vaultPoolContract as any, 'withdrawByAmount', [convertedStakeAmount.toString()], callOptions)
    })
  ...
  const handleDeposit = async (lockDuration = 0) => {
    const receipt = await fetchWithCatchTxError(() => {
      ...
      return callWithGasPrice(vaultPoolContract as any, 'deposit', methodArgs, callOptions)
    })
